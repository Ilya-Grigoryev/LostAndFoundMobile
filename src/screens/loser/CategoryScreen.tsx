...
export default function CategoryScreen() {
  ...
  return (
    <View style={styles.root}>
      <ScreenHeader
        title={t('loser.category.title')}
        onBack={() => nav.getParent()?.goBack()}
        accentColor={colors.loserPrimary}
      />
      <View style={styles.body}>
        <Text style={typography.body}>Category (stub)</Text>
        <Button label="Next" color={colors.loserPrimary} onPress={() => nav.navigate('LocationMode')} />
      </View>
    </View>
  );
}
